...
/** npm 包名格式校验（含 @scope/name；旧式大写包名也兼容） */
const NPM_PACKAGE_NAME_PATTERN = /^(?:@[a-z\d](?:[a-z\d._-]*[a-z\d])?\/)?[a-z\d](?:[a-z\d._-]*[a-z\d])?$/iu;
...
/** 从 npm 包名生成安装命令；包名必须符合 npm 命名规则（防 README/package.json 注入换行等）。 */
export function npmInstallCommandFor(packageName) {
  if (typeof packageName !== "string") return null;
  const normalized = packageName.trim();
  if (!normalized || normalized.length > 214) return null;
  if (!NPM_PACKAGE_NAME_PATTERN.test(normalized)) return null;
  return `dsh plugin --profile web add ${normalized}`;
}
